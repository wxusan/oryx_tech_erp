...
>
> **Update (split-payment amount-entry fix)**: the split-payment UI built for
> items 12/13 (nasiya and sale payment modals) had a real UX/logic bug: the
> single "Miqdor" field meant the TOTAL payment, and the visible second field
> ("{method} summasi") actually meant the FIRST part's amount, with the true
> second part computed silently as `total - firstPart`. A user entering
> `$100` in "Miqdor" and `$40` in the visible field got Naqd=$60/Karta=$40 —
> not the $100/$40 they thought they were splitting. **Fixed**: each payment
> method now has its own independent amount field ("To'lov usuli 1" /
> "To'lov usuli 2"), and the total shown is always the calculated, read-only
> **sum** of the two parts (`paymentTotal = part1.amount + part2.amount`),
> never a total-minus-second-part subtraction. The recommended-amount button
> fills part 1 and leaves part 2 empty rather than filling a "total" field.
> Backend `validatePaymentBreakdown` also now rejects a missing method and
> duplicate methods across parts (previously only checked positive amounts
> and sum-equals-total). Payment history now shows each part's own amount
> inline (e.g. "Naqd: $60.00" / "Karta: $40.00") instead of hiding amounts in
> a hover-only tooltip. Telegram's split-payment message was **already
> correct** (it always rendered each part's stored amount independently, not
> a recomputed total) — no Telegram template change was needed. Files:
> `src/components/shop/nasiya-payment-modal.tsx`,
> `src/app/(shop)/shop/qurilmalar/[id]/page.tsx`,
> `src/lib/payment-breakdown.ts`,
> `src/app/(shop)/shop/nasiyalar/[id]/page.tsx`. Tests:
> `tests/split-payment.guard.test.ts` (rewritten, 36 tests),
> `tests/payment-breakdown.test.ts` (+3 tests: duplicate method, missing
> method, total-disagrees-with-parts regression).
>
