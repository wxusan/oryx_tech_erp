...
| **Ombordagi tannarx** | `inventoryPurchaseCost` = `Σ Device.purchasePrice` where `status = IN_STOCK` | Devices currently in stock | Sold/returned/deleted devices — the moment a device's status changes from IN_STOCK (for example, on sale), the next fetch of this aggregate excludes it automatically | N/A (point-in-time status, not date-ranged) | Legacy-UZS purchase price, one-time frozen conversion per device — safe to sum across devices bought at different rates (see `docs/currency-accounting-model.md` §11) | `invalidateShopDeviceMutation`, `invalidateShopSaleMutation`, `invalidateShopNasiyaMutation`, `invalidateShopReturnMutation` |
| **Sotuvlar** | `soldThisMonth` = count of `cashSalesThisMonth` rows | Every Sale created this month, including fully paid and Qarz/Olib rows | Nasiyas (tracked separately as "Faol nasiyalar") | `Sale.createdAt` | N/A (a count) | `invalidateShopSaleMutation` |
