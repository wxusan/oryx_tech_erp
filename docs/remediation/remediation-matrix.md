...
| F-003 | Initial payment metadata and some exports/reminders/Olib use legacy/current-rate values | sale/nasiya/Olib create routes, exports, cron, UI | Write frozen input metadata and use native contract read model everywhere | Rate-change golden tests, CSV/XLSX/message tests | Data audit; optional additive backfill markers only | High/accounting | OPEN |
| F-004 | Stock picker defaults to 200; Olib has no real pagination | device picker pages, devices API, Olib API/UI | Server-searchable paginated selector and Olib pagination | 100k-last-record lookup, paging/filter behavior | Index only if query plan proves necessary | Medium/UX | OPEN |
| F-005 | Filtered nasiya and overdue/dashboard paths load unbounded schedules/debts | shop lists, due-overdue API, shop stats | Bounded SQL/queryable derived-status projection | Status equivalence, query bounds, performance | Likely additive projection/index | High/correctness | OPEN |
...
| F-014 | Duplicate admin/shop/currency reads; capped admin aggregate; trust over-fetch | admin stats/page, customer API, currency helper | Focused read models and request memoization | Query count, >2k shops, mature customer | Possible summary fields/index | Medium | OPEN |
