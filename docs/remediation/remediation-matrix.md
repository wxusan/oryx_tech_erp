...
| F-002 | Password minimum raised to ten; login throttle remains per instance until distributed failure semantics are implemented | `auth.ts`, admin routes, rate-limit adapter | Finish distributed login/source throttling | Multi-instance limiter, password create/reset/login | None unless auth-event persistence is approved | Medium/security | IN PROGRESS |
...
| F-015 | Olib mobile cards and real shop identity added; admin identity and route resilience remain | Olib UI, layouts, route segments | Finish server-seeded admin identity and errors/loaders | 390/768/desktop and failure states | None | Low/medium | IN PROGRESS |
