...
Run `scripts/sql/device-specs-phone-repair-diagnostics.sql` read-only and approve repairs separately for ambiguous storage, unknown condition, placeholder/invalid/colliding legacy IMEIs, and phone collisions. Production health, public login rendering, responsive widths and authorization redirects are proven; authenticated mutation flows, Telegram delivery against the real bot and production query latency still require operator credentials/traffic and are not claimed here.

## 20. Production/deployment status

Online. PR #4 merged the incremental cache into `main` at `335bfff`. PR #6 merged this remediation into `main` at `73d1f35` on 2026-07-12. Before the primary alias changed, Vercel ran the official `prisma migrate deploy` command against production and successfully applied migrations `202607120001` through `202607120004`; the repository's ordinary `vercel.json` build command remained `npm run build`. Vercel deployment `dpl_7dw8N3RpgrPErKeHZAtHyNb7g5PG` is READY and aliases `https://oryx-tech-erp.vercel.app`. `/api/health` reports commit `73d1f35` and `database=ok`; recent production error logs were empty. Production data repair remains a separate approval step after diagnostics.
