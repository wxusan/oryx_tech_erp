...
- **Schedule:** `*/10 * * * *` (every 10 min, UTC) — see `vercel.json`.
...
2. **Sub-daily cron requires Vercel Pro** (or an external scheduler hitting the endpoint every ~10 min with `CRON_SECRET`). On a plan that only runs cron daily, the jitter window still works but delivery batches less finely — documented in `docs/cron-jobs.md`.
