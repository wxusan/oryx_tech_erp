...
describe('production release guard', () => {
  ...
  it('runs count-only read-only checks before release and verifies all migrations afterward', () => {
    ...
    expect(preflightScript).toContain('pending_subscription_payment_currency_reconstruction')
    expect(preflightScript).toContain('subscription_payment_native_snapshot_issues')
    expect(preflightScript).toContain('active_legacy_write_off_permission')
    expect(preflightScript).toContain('202607150004_complete_accounting_redesign')
    expect(preflightScript).toContain('202607180001_telegram_disable_lifecycle')
    expect(preflightScript).toContain('202607200001_olib_nasiya_debt_ledger')
    expect(preflightScript).toContain('202607220001_custom_shop_staff_roles')
    expect(preflightScript).toContain('202607220002_nasiya_early_settlement')
    expect(preflightScript).toContain('202607220003_nasiya_return_refund')
    expect(preflightScript).toContain('202607230001_return_currency_refund_method')
    expect(preflightScript).toContain('nasiya_return_permission_definition_missing')
    expect(preflightScript).toContain('return_currency_method_schema_issues')
