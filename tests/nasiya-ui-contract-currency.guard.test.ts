...
describe('nasiya detail page shows the frozen contract currency', () => {
  const page = read('src/app/(shop)/shop/nasiyalar/[id]/page.tsx')

  it('shows a "Shartnoma: ..." note only when the contract currency differs from the shop\'s display currency', () => {
    expect(page).toContain('nasiya.contractCurrency !== currency.currency')
    expect(page).toContain('Shartnoma:')
    expect(page).toContain('formatContractMoney(nasiya.contractFinalAmount, nasiya.contractCurrency)')
  })
...
describe('nasiya payment modal previews the contract-currency applied amount', () => {
  ...
  it('shows a "Shartnomaga qo\'llanadi" preview only when payment currency differs from contract currency, using convertPaymentToContractCurrency', () => {
    expect(modal).toContain('contractCurrency !== currency.currency && currency.usdUzsRate')
    expect(modal).toContain('convertPaymentToContractCurrency(Number(payAmount) || 0, currency.currency, contractCurrency, currency.usdUzsRate)')
    expect(modal).toContain('Shartnomaga qo&apos;llanadi:')
  })
