...
function session(
  ...
  packageVersionId = role === 'SHOP_ADMIN' ? 'package-1' : null,
  sessionPolicy: 'IDLE_10_MINUTES' | 'REMEMBERED_30_DAYS' = role === 'SUPER_ADMIN' ? 'IDLE_10_MINUTES' : 'REMEMBERED_30_DAYS',
) {
  return {
    user: {
...
function durable(
  ...
  packageVersionId = role === 'SHOP_ADMIN' ? 'package-1' : null,
  policy: 'IDLE_10_MINUTES' | 'REMEMBERED_30_DAYS' = role === 'SUPER_ADMIN' ? 'IDLE_10_MINUTES' : 'REMEMBERED_30_DAYS',
) {
  return {
...
describe('durable session authorization', () => {
  ...
  it('does not apply inactivity logout to a remembered shop session', async () => {
  ...
  it('applies the ten-minute server idle deadline to an unchecked shop session', async () => {
    mocks.auth.mockResolvedValue(session('SHOP_ADMIN', 'package-1', 'IDLE_10_MINUTES'))
    mocks.authSessionFind.mockResolvedValue(durable(
      'SHOP_ADMIN',
      new Date(Date.now() - 10 * 60 * 1000 - 1),
      'package-1',
      'IDLE_10_MINUTES',
    ))
    const { requireApiSession } = await import('@/lib/api-auth')
    ...
    expect(mocks.shopAdminFind).not.toHaveBeenCalled()
  })

  it('revokes a session bound to a package that is no longer active', async () => {
    mocks.auth.mockResolvedValue(session('SHOP_ADMIN', 'package-1'))
    mocks.authSessionFind.mockResolvedValue(durable('SHOP_ADMIN', new Date(), 'package-1'))
    mocks.shopAdminFind.mockResolvedValue({
      shopId: 'shop-1',
      sessionVersion: 3,
      permissionVersion: 1,
    ...
    expect(result.ok).toBe(false)
    if (!result.ok) await expect(result.response.json()).resolves.toMatchObject({ error: expect.stringContaining('paketi o\'zgargan') })
    expect(mocks.authSessionUpdate).toHaveBeenCalledWith(expect.objectContaining({ data: { revokedAt: expect.any(Date) } }))
  })

  it('revokes a staff session when STAFF_ACCESS is disabled', async () => {
    mocks.auth.mockResolvedValue(session('SHOP_ADMIN', 'package-2'))
    mocks.authSessionFind.mockResolvedValue(durable('SHOP_ADMIN', new Date(), 'package-2'))
    mocks.shopAdminFind.mockResolvedValue({
      shopId: 'shop-1',
      sessionVersion: 3,
      permissionVersion: 1,
