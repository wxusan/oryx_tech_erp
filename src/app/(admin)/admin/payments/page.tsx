...
interface ShopPaymentItem {
  ...
  amountUzsSnapshot: number | null
  amountUsdSnapshot: number | null
  currencyReconstructionStatus: 'PENDING' | 'COMPLETE' | 'PARTIAL' | 'UNRECONSTRUCTABLE'
  months: number
...
type PaymentPeriodSummary = HistoricalAdminMoney
...
export default function PaymentsPage() {
  ...
  const payments: Payment[] = (paymentsQuery.data?.items ?? []).map((payment) => ({
    id: payment.id,
    shop: payment.shop,
    displayAmount: formatAdminPaymentRow(payment, currency),
    originalAmount: formatUserFacingMoney({
  ...
  const statCards = [
    { label: "Bu oy jami", value: summary ? formatHistoricalAdminMoney(summary.currentMonth, currency) : '—', sub: `${summary?.currentMonth.count ?? 0} ta to'lov` },
    { label: "O'tgan oy", value: summary ? formatHistoricalAdminMoney(summary.previousMonth, currency) : '—', sub: `${summary?.previousMonth.count ?? 0} ta to'lov` },
    { label: "Yil davomida", value: summary ? formatHistoricalAdminMoney(summary.currentYear, currency) : '—', sub: summary ? `${summary.currentYearNumber}-yil` : 'Yuklanmoqda...' },
  ]
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <a href="/api/admin/payments?format=csv" className="inline-flex h-9 items-center gap-2 border border-zinc-200 px-3 text-sm text-zinc-700 hover:bg-zinc-50">
            ...
            {loading ? (
              ...
              payments.map((p) => (
                <TableRow key={p.id} className="border-zinc-100 hover:bg-zinc-50">
                  ...
                  <TableCell className="text-sm font-semibold text-zinc-800">
                    {p.displayAmount}
                    {p.displayAmount !== p.originalAmount && <span className="block text-[10px] font-normal text-zinc-400">Asl: {p.originalAmount}</span>}
                  </TableCell>
