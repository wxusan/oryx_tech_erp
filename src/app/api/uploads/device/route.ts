...
export async function POST(request: Request) {
  ...
  try {
    ...
    const reference = createPrivateUploadReference({ key, shopId, kind: 'device' })
    return ok({
      reference,
      url: new URL(privateUploadPreviewUrl('device', reference), request.url).toString(),
    })
  } catch (error) {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = readPrivateUploadReference({ reference, kind: 'device' })
